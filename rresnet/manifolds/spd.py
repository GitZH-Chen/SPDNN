...
from spnn.functionals import sym_expm,sym_invsqrtm2
...
class SPD(Manifold):
    ...
    def __init__(self, metric="AIM"):
        super().__init__()
        self.wmin = 1e-8
        self.wmax = 1e8
        self.metric = metric # "AIM" or "LEM"
    ...
    def exp(self, x, u, t=1):
        if self.metric == "AIM":
            sqrt_x, inv_sqrt_x = sym_invsqrtm2.apply(x)
            x_new = sqrt_x @ sym_expm.apply(inv_sqrt_x @ u @ inv_sqrt_x) @ sqrt_x
            return x_new

        elif self.metric == "LEM":
            return sym_expm.apply(self.log(x) + t * u)
